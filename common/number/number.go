...
func Trim(s string) string {
	...
	s = regexp.MustCompile(`(?i)[-_\s]*(carib(b?ean)?(com)?(pr)?|1?Pond?o?|10musume|pacopacomama|muramura|Tokyo[-_\s]?Hot)([-_\s]*|$)`).
...
func IsUncensored(s string) bool {
	return regexp.
		MustCompile(`^(?i)([\d-]{4,}|\d{6}_\d{2,3}|(cz|gedo|k|n|kb|red-|se)\d{2,4}|(heyzo|xxx-av|heydouga|kin8)[-_].+)$`).
		MatchString(s)
}
